...
def test_refresh_cn_stock_reuses_industry_column_without_second_source_call(monkeypatch, db_session):
    frame = pd.DataFrame([
        {"代码": "600001", "名称": "测试银行", "所属行业": "银行"},
    ])
    monkeypatch.setattr(universe_sync, "_fetch_cn_stock_list", lambda db: frame)

    def unexpected_second_fetch(*args, **kwargs):
        raise AssertionError("industry source should be reused from the stock list")

    monkeypatch.setattr(universe_sync, "_fetch_cn_stock_industry_map", unexpected_second_fetch)

    result = universe_sync.refresh_universe_symbols("cn-stock", db_session)

    item = db_session.execute(
        select(UniverseSymbol).where(UniverseSymbol.symbol == "600001")
    ).scalar_one()
    assert result["created"] == 1
    assert item.industry == "银行"


def test_fetch_cn_stock_industry_map_strict_rejects_empty_source(monkeypatch, db_session):
    monkeypatch.setattr(
        universe_sync,
    ...
    with pytest.raises(RuntimeError, match="返回空数据"):
        universe_sync._fetch_cn_stock_industry_map(db_session, strict=True)


def test_backfill_universe_stock_industries_only_fills_blank_cn_stocks(monkeypatch, db_session):
    blank = UniverseSymbol(
        symbol="600101", name="Blank", asset_type="stock", market="sh", region="cn",
        industry=None, is_synced=0, bar_count=1,
    )
    existing = UniverseSymbol(
        symbol="600102", name="Existing", asset_type="stock", market="sh", region="cn",
        industry="保险", is_synced=1, bar_count=2,
    )
    etf = UniverseSymbol(
        symbol="510300", name="ETF", asset_type="etf", market="sh", region="cn",
        industry=None, is_synced=0,
    )
    us_stock = UniverseSymbol(
        symbol="AAPL", name="Apple", asset_type="stock", market="us", region="us",
        industry=None, is_synced=0,
    )
    db_session.add_all([blank, existing, etf, us_stock])
    db_session.commit()
    db_session.refresh(blank)
    db_session.refresh(existing)
    db_session.add(UniverseDailyBar(
        universe_symbol_id=blank.id,
        trade_date=date(2026, 8, 1),
        close=10,
    ))
    db_session.commit()

    def fake_industry_fetch(db, *, strict=False):
        return {"600101": "生物医药", "600102": "银行", "510300": "基金"}

    monkeypatch.setattr(universe_sync, "_fetch_cn_stock_industry_map", fake_industry_fetch)

    result = universe_sync.backfill_universe_stock_industries()

    db_session.expire_all()
    refreshed_blank = db_session.get(UniverseSymbol, blank.id)
    refreshed_existing = db_session.get(UniverseSymbol, existing.id)
    assert result == {"total": 2, "processed": 2, "updated": 1, "skipped": 1, "failed": 0}
    assert refreshed_blank is not None
    assert refreshed_blank.industry == "生物医药"
    assert refreshed_blank.is_synced == 0
    assert refreshed_blank.bar_count == 1
    assert db_session.execute(
        select(UniverseDailyBar).where(UniverseDailyBar.universe_symbol_id == blank.id)
    ).scalar_one().close == 10
    assert refreshed_existing is not None
    assert refreshed_existing.industry == "保险"
    assert db_session.get(UniverseSymbol, etf.id).industry is None
    assert db_session.get(UniverseSymbol, us_stock.id).industry is None
