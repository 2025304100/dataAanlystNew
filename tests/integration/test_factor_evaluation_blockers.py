...
class SyntheticMockBase:
    ...
    def apply_synthetic_patchers(
        ...
        sample_signal_dates = [
            ...
            for i in range(10, 10 + n_trade_dates)
        ]
        symbols = [f"SH{i:06d}" for i in range(600000, 600500)]
        rows = []
        for sd in sample_signal_dates:
            for s in symbols[:50]:
        ...
        ctd_obj.observed_symbols = 500
        ctd_obj.expected_symbols = 500
        ctd_obj.completeness_ratio = 1.0
        ctd_obj.fallback_reason = None
        ...
        # Values persisted by the real panel outcome must be concrete rather
        # than implicit MagicMock attributes.
        panel_outcome.observed_symbols = len(symbols_p)
        panel_outcome.expected_symbols = len(symbols_p)
        panel_outcome.completeness_ratio = 1.0
        panel_outcome.fallback_reason = None
