...
class TestFactorExecutorPreview:
    ...
    def test_preview_ep_formula_requires_daily_spine(self, tmp_warehouse):
        """EP 公式：**只 seed 估值、没有日线** → 明确报「缺日线对齐基准」。

        2026-09-17 需求方裁决 ③：`raw_daily_bars` 是因子计算的**对齐基准**
        （universe × date 网格），估值/财报类公式也必须挂在日线上才能按交易日
        对齐、算覆盖率、与目标收益对齐。缺它时**不许静默返回空值**，
        而要给结构化、可操作的报错。

        （本用例原先断言「只 seed 估值就能算出值」—— 那是**错误的前提**：
         没有网格就没有可对齐的截面。已按裁决改写为「应报错」，
         并在 `test_preview_ep_formula_with_spine` 里给出「补上日线就正常」的对照。）
        """
        valuation_rows = [
        ...
        assert outcome.values == [], "缺日线时不得产出值"
        assert any("no_source_data" in e for e in outcome.errors), outcome.errors
        joined = " ".join(outcome.errors)
        # 报错必须**可操作**：指出缺哪张表、依赖字段、以及去哪儿补
        assert "raw_daily_bars" in joined
        assert "pe_ttm" in joined, "应提示本公式依赖的字段"
        assert "镜像" in joined, "应指引用户去补数据"

    def test_preview_ep_formula_with_spine(self, tmp_warehouse):
        """对照：补上日线后，EP 公式正常算出值（证明不是功能缺失）。"""
        _seed_daily_bars(tmp_warehouse, [
            {"symbol": "000001", "trade_date": "2026-07-24", "adjust": "qfq",
             "close": 10.0, "open": 9.5, "high": 10.5, "low": 9.0,
             "volume": 1000.0, "amount": 10000.0, "turnover_rate": 0.05,
        ...
        assert len(outcome.values) == 2
        by_symbol = {str(v["symbol"]): v["raw_value"] for v in outcome.values}
        assert abs(by_symbol["000001"] - 0.1) < 1e-10
        assert abs(by_symbol["000002"] - 0.05) < 1e-10

    def test_preview_conditional_formula_requires_daily_spine(self, tmp_warehouse):
        """条件公式：同样必须报「缺日线对齐基准」（裁决 ③，见上一个用例的说明）。"""
        _seed_valuation(tmp_warehouse, [
            {"symbol": "000001", "trade_date": "2026-07-24", "pe_ttm": 10.0},
        ...
        assert any("no_source_data" in e for e in outcome.errors), outcome.errors
        assert "raw_daily_bars" in " ".join(outcome.errors)

    def test_preview_conditional_formula_with_spine(self, tmp_warehouse):
        """对照：补上日线后，条件分支正常求值（含 `else 0` 分支）。"""
        _seed_daily_bars(tmp_warehouse, [
            {"symbol": "000001", "trade_date": "2026-07-24", "adjust": "qfq",
             "close": 10.0, "open": 9.5, "high": 10.5, "low": 9.0,
             "volume": 1000.0, "amount": 10000.0, "turnover_rate": 0.05,
        ...
        assert len(outcome.values) == 2
        by_symbol = {str(v["symbol"]): v["raw_value"] for v in outcome.values}
        assert by_symbol["000001"] == 0.1        # pe_ttm=10 > 0 → 1/10
        assert by_symbol["000002"] == 0.0        # pe_ttm=-5 → else 0
...
class TestFactorExecutorExecute:
    ...
    def test_execute_empty_data_returns_error(self, tmp_warehouse):
        ...
        assert len(outcome.errors) > 0
        # 错误串现在是**结构化可操作**的（裁决 D-G）：仍以 `no_source_data`
        # 开头，但整体是一句带指引的话 —— 故用子串判定而非列表精确成员判定
        assert any("no_source_data" in e for e in outcome.errors), outcome.errors
        assert any("raw_daily_bars" in e for e in outcome.errors), \
            "报错应指出缺哪张表（结构化、可操作）"
