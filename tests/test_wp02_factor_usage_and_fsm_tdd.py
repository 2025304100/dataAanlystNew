...
class TestWP02FactorUsageAtomicBehavior:
    """TD7 行为级：真实 sqlite（auto-align 全表、PRAGMA foreign_keys=ON）跑原子 7 步 DB 路径。

    背景：原 A1~A5 仅签名级验证，save_and_apply_usage_atomic 的 DB 路径从未被执行——
    snapshot（varchar PK 无 id 生成）+ int(snap.id) 契约坏点直到 TD7 才暴露。
    """
    ...
    def _seed(db):
        ...
        p = Portfolio(
            name="TD7 atomic behavior", account_type="sim", asset_scope="mixed",
            total_capital=1_000_000.0, investable_ratio=1.0, cash_reserve_ratio=0.05,
            currency="CNY", is_default=0, buy_fee_pct=0.00025, sell_fee_pct=0.00025,
            benchmark_code="000300", default_single_position_pct=0.3,
            auto_trade_enabled=0,
        )
        db.add(p)
        db.flush()
        # factor_sets.id 是 String PK；子表 factor_set_id 为 Integer，sqlite FK 亲和按父列转换，用 "1" 可匹配
        fs = FactorSet(
            id="1", name="td7 fs", status="draft", description="td7",
            content_hash="td7-hash",
        )
        db.add(fs)
        db.commit()
        return p
    ...
    def _payload():
        return {
            ...
            "factor_weights": {"alpha": 0.6, "beta": 0.4},
        }

    def test_t7_01_happy_path_snapshot_id_is_server_generated_string(self, db_session):
        ...
        p = self._seed(db_session)
        result = save_and_apply_usage_atomic(
            portfolio_id=p.id,
            payload=self._payload(),
            expected_row_version=0,
            idempotency_key="td7-key-001",
            db=db_session,
            correlation_id="corr-td7-001",
            operator_id="qa_user",
        )
        # snapshot_id 契约：非空字符串、可反查、行内 id 一致
        assert isinstance(result.snapshot_id, str) and result.snapshot_id
        snap = db_session.get(StrategyExecutionSnapshot, result.snapshot_id)
        assert snap is not None
        assert snap.snapshot_no == 1
        assert snap.usage_binding_id == result.usage_id
        assert snap.portfolio_id == p.id
        assert snap.snapshot_hash and len(snap.snapshot_hash) == 64
        assert snap.decision_clock_json and "pfu_clock_v1" in snap.decision_clock_json
        # usage 落数（单数表，row_version 乐观锁起步）
        u = db_session.get(PortfolioFactorUsage, result.usage_id)
        assert u is not None and int(u.row_version) == 1
        # outbox payload 的 snapshot_id 同为字符串且与结果一致
        evs = db_session.execute(
            select(OutboxEvent).where(
                OutboxEvent.event_type == "PORTFOLIO_FACTOR_USAGE_APPLIED",
                OutboxEvent.portfolio_id == p.id,
            ),
        ).scalars().all()
        assert evs, "outbox 必须落事件"
        payload = json.loads(evs[0].payload_json)
        assert payload["snapshot_id"] == result.snapshot_id

    def test_t7_02_idempotent_replay_returns_same_string_snapshot_id(self, db_session):
        from app.services.portfolio_factor_usage import save_and_apply_usage_atomic
        ...
        kw = dict(
            ...
            payload=self._payload(),
            expected_row_version=0,
            idempotency_key="td7-key-002",
            db=db_session,
        )
        r1 = save_and_apply_usage_atomic(correlation_id="corr-a", **kw)
        r2 = save_and_apply_usage_atomic(correlation_id="corr-b", **kw)  # 同 key 同 hash → 重放
        assert r2.snapshot_id == r1.snapshot_id
        assert isinstance(r2.snapshot_id, str)
        assert r2.usage_id == r1.usage_id and r2.row_version == r1.row_version

    def test_t7_03_injected_failure_midstep_leaves_no_side_effect(self, db_session):
        from app.services.portfolio_factor_usage import (
            save_and_apply_usage_atomic, SAVE_STEP_WRITE_OUTBOX,
        )
        from app.models.decision_engine import StrategyExecutionSnapshot
        from app.models.portfolio_factor_usage import PortfolioFactorUsage
        ...
        def boom(step, ctx):
            if step == SAVE_STEP_WRITE_OUTBOX:
                raise RuntimeError("td7 injected failure")

        with pytest.raises(RuntimeError, match="td7 injected failure"):
            save_and_apply_usage_atomic(
                portfolio_id=p.id,
                payload=self._payload(),
                expected_row_version=0,
                idempotency_key="td7-key-003",
                db=db_session,
                inject_failure=boom,
            )
        # 回滚后无任何副作用（绑定/快照/幂等记录都不存在）
        assert db_session.execute(
            select(func.count()).select_from(PortfolioFactorUsage)
            .where(PortfolioFactorUsage.portfolio_id == p.id),
        ).scalar_one() == 0
        assert db_session.execute(
            select(func.count()).select_from(StrategyExecutionSnapshot)
            .where(StrategyExecutionSnapshot.portfolio_id == p.id),
        ).scalar_one() == 0
        assert db_session.get(StrategyExecutionSnapshot, "td7-nonexistent") is None
