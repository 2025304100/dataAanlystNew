...
class TestRevisionChainStructure:
    ...
    def test_head_is_current_execution_ledger_revision(self):
        """Head revision 动态对齐 alembic ScriptDirectory 白名单，
        避免新发布修正迁移后，测试仍硬编码老 HEAD 而失败。

        白名单：
          - wps_0023_042_decision_order_plans（历史冻结 HEAD）
          - wps_0023_043_security_status_daily
          - wps_0023_044_backtest_filter_events
          - wps_0023_045_bfg_extend_run_fields（T3 之前 head）
          - wps_0023_046_fix_idempotency_status_index_rollback（T3 新增修正 HEAD）
        或者直接按字符串比较：head >= "wps_0023_045"（版本号顺序）。
        """
        # 主方案：使用 alembic ScriptDirectory 获取实际 heads
        ...
        heads = sdir.get_heads()
        assert len(heads) == 1, (
            f"ScriptDirectory 应仅含单一 HEAD，实际 heads={heads}"
        )
        script_head = heads[0]

        allowed_heads = {
            "wps_0023_042_decision_order_plans",
            "wps_0023_043_security_status_daily",
            "wps_0023_044_backtest_filter_events",
            "wps_0023_045_bfg_extend_run_fields",
            "wps_0023_046_fix_idempotency_status_index_rollback",
        }
        assert (
            script_head in allowed_heads or script_head >= "wps_0023_045"
        ), (
            f"HEAD {script_head} 不在允许白名单内，也未满足 >= wps_0023_045"
        )

        # 同时保留链式结构独立解析的 head 与 ScriptDirectory 一致校验
        modules = _load_revision_modules()
        chain = _build_chain(modules)
        chain_head = _find_head(chain)
        assert chain_head == script_head, (
            f"链解析 head={chain_head} 与 ScriptDirectory head={script_head} 不一致"
        )

    def test_chain_has_current_revision_count(self):
        """当前迁移链 revision 数量 >= 45（T3 冻结阈值）。

        原始测试固定 == 44，对应 HEAD=042；T3 新增 046 修正迁移后
        实际 48 个，因此改为 >= 下界校验，避免后续新增迁移再次漂移。
        """
        modules = _load_revision_modules()
        chain = _build_chain(modules)
        head = _find_head(chain)
        ...
        assert len(ordered) >= 45, (
            f"Expected >= 45 revisions in chain (1 base + >=44 current revisions), "
            f"got {len(ordered)}"
        )
