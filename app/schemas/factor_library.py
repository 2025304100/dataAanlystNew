...
class FactorSetMemberCreate(BaseModel):
    ...
    factor_version_id: int | None = None
    # UI-facing lookup fields. The route resolves these to stable internal IDs
    # before calling the service, so users never need to see database IDs.
    ...
    def require_member_reference(self):
        has_ids = self.factor_id is not None and self.factor_version_id is not None
        has_code = bool(self.factor_code) and self.factor_version is not None
        if not (has_ids or has_code):
            raise ValueError(
                "provide factor_id + factor_version_id or factor_code + factor_version"
            )
        return self
