...
class FactorPipelineCreate(BaseModel):
    ...
    # 审计/透传字段（Task 9 三处一致写入）
    actor: str = "settings:pipeline"
    source_hint: str = "settings_run"
    mirror_only: bool = False
    batch_size: int = 50000
    scope: str = "recent"
    # Task 9 (3) 三处一致写入 — ②③ 两处 dict（均含 factor_set_id）
    hyperparameters_json: dict | None = Field(
        default=None,
        description="训练超参数容器；train=true 时 factor_set_id 必须等于顶层 payload.factor_set_id。",
    )
    factor_scope_json: dict | None = Field(
        default=None,
        description="因子作用域容器；train=true 时 factor_set_id 必须等于顶层 payload.factor_set_id。",
    )
