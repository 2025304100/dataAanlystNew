...
# ---------- Task 12.1: 统一数据截止信息（与 bfg_precheck.DataCutoffInfo 字段对齐） ----------
class DataCutoffInfo(BaseModel):
    model_config = ConfigDict(extra="forbid")

    bars_date: Optional[date] = None
    status_date: Optional[date] = None
    factors_date: Optional[date] = None
    unified_earliest: Optional[date] = None
    sync_at: Optional[datetime] = None
    version: Optional[str] = None


# ---------- Task 12.2: 7 要素结构化错误响应 ----------
class StructuredError(BaseModel):
    """HTTP 非 2xx 统一错误体。路由 try/except 或全局 handler 统一返回。"""
    ...
    # T1 兼容：保留 PRECHECK_BLOCKED 的原有 blockers 字段
...
# ---------- BFG: BacktestFilterConfig DTO ----------
class BacktestFilterConfigDTO(BaseModel):
    """回测过滤治理配置（Pydantic），与 app.services.backtest_filters.config.BacktestFilterConfig frozen dataclass 字段一一对应。"""
    model_config = ConfigDict(extra="forbid")

    filter_new_listing: bool = Field(default=True, description="是否启用次新股过滤")
    min_listing_age_calendar_days: int = Field(default=120, ge=1, le=3650, description="次新股最小上市自然日数")
    filter_st: bool = Field(default=True, description="是否启用 ST/*ST 过滤")
    filter_suspended: bool = Field(default=True, description="是否启用停牌过滤")
    force_delisting_liquidation: bool = Field(default=True, description="是否启用退市强制清算")
    delisting_period_excluded: bool = Field(default=True, description="是否剔除退市整理期新开仓")
    min_history_days: int = Field(default=20, ge=1, le=1000, description="最小历史数据天数门槛")
    production_fidelity: bool = Field(default=True, description="True=生产保真；关闭核心规则时会被自动覆写为 False")
...
class BacktestRunRequest(BaseModel):
    ...
    filter_config: BacktestFilterConfigDTO | None = Field(
        default=None,
        description="可选过滤治理配置；不传则使用 BacktestFilterConfig 默认生产保真配置",
    )
...
class BacktestRunDetail(BacktestRunRead):
    ...
    # ---------- Task 12.1: 数据截止日期信息 ----------
    data_cutoff: DataCutoffInfo | None = None
    cutoff_mismatch: bool = False
    cutoff_warning_zh: str | None = None
