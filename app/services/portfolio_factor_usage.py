...
def save_and_apply_usage_atomic(
    *,
    ...
    try:
        ...
        snap_hash = content_hash("pfu_snap", snap_json, correlation_id)
        applied_now = datetime.now(timezone.utc)
        # TD7：id 服务端确定性生成（PK 是 varchar(64) 无自增；现役 factor_usage_service.py 同款模式），
        # 并补齐 NOT NULL 列（decision_clock_json/member_snapshot_json/snapshot_hash/effective_from）
        snap = StrategyExecutionSnapshot(
            id=content_hash("ses", snap_hash, correlation_id),
            portfolio_id=int(portfolio_id),
            usage_binding_id=int(pfu.id),
            snapshot_no=next_snapshot_no,
            snapshot_json=snap_json,
            content_hash=content_hash(snapshot_payload),
            decision_clock_json=canonical_json({
                "schema": "pfu_clock_v1",
                "decision_at_utc": applied_now.isoformat(),
                "applied_via": "save_and_apply_usage_atomic",
            }),
            member_snapshot_json=snap_json,
            snapshot_type="save_and_apply",
            snapshot_hash=snap_hash,
            effective_from=applied_now,
            factor_set_id=factor_set_id,
            factor_model_run_id=factor_model_run_id,
