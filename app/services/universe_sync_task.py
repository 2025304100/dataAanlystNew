...
UNIVERSE_INDUSTRY_BACKFILL_TASK_TYPE = "universe_industry_backfill"
...
def _is_cancelled(task_id: str) -> bool:
    ...
    try:
        ...
        if task is None:
            return True
        return task.status == "cancelled" or int(getattr(task, "cancel_requested", 0) or 0) == 1
    finally:
        db.close()
        ...
        existing = db.execute(
            select(AsyncTaskRecord).where(
                AsyncTaskRecord.task_type.in_((
                    ...
                    UNIVERSE_INDUSTRY_BACKFILL_TASK_TYPE,
                )),
...
def start_universe_industry_backfill() -> AsyncTaskRead:
    """启动一次性 A 股股票行业字段补齐任务。"""
    db = SessionLocal()
    try:
        existing = db.execute(
            select(AsyncTaskRecord).where(
                AsyncTaskRecord.task_type.in_(tuple([
                    UNIVERSE_SYNC_TASK_TYPE,
                    UNIVERSE_INCREMENTAL_TASK_TYPE,
                    UNIVERSE_BACKFILL_TASK_TYPE,
                    UNIVERSE_SMART_TASK_TYPE,
        ...
        task = AsyncTaskRecord(
            id=task_id,
            task_type=UNIVERSE_INDUSTRY_BACKFILL_TASK_TYPE,
            status="queued",
            stage="queued",
            percent=0,
            message="行业信息补齐任务已创建（仅 A 股股票）",
            payload_json=json.dumps({"mode": "industry_backfill", "scope": "cn-stock"}, ensure_ascii=False),
        )
        db.add(task)
        db.commit()
        db.refresh(task)
        ...
        db.close()
    threading.Thread(target=_run_universe_industry_backfill, args=(task_id,), daemon=True).start()
    return result


def get_latest_universe_industry_backfill_task() -> AsyncTaskRead | None:
    db = SessionLocal()
    try:
        task = db.execute(
            select(AsyncTaskRecord)
            .where(AsyncTaskRecord.task_type == UNIVERSE_INDUSTRY_BACKFILL_TASK_TYPE)
...
def cancel_universe_industry_backfill_task(task_id: str) -> AsyncTaskRead | None:
    db = SessionLocal()
    try:
        task = db.get(AsyncTaskRecord, task_id)
        if task is None:
...
def _run_universe_industry_backfill(task_id: str) -> None:
    SessionFactory = get_session_local()
    result: dict[str, Any] = {}
    ...
    try:
        db = SessionFactory()
        try:
            _set_task(db, task_id, status="running", stage="industry_backfill", percent=0,
                      message="开始补齐 A 股股票行业信息", started_at=_now())
        finally:
            db.close()

        # 取消可能发生在任务入队后、worker 初始化期间；此时不应再访问数据源。
        if _is_cancelled(task_id):
            return

        def progress(processed: int, total: int, updated: int, failed: int) -> None:
            db = SessionFactory()
            try:
                _set_task(db, task_id, percent=round(processed / total * 100, 1) if total else 100,
                          total=total, processed=processed, ok_count=updated, failed_count=failed,
                          message=f"行业信息补齐中 {processed}/{total}（已更新 {updated}）")
            finally:
                db.close()
        ...
        error = exc
        logger.exception("universe industry backfill failed")
    db = SessionFactory()
    try:
        cancelled = _is_cancelled(task_id)
        status = "cancelled" if cancelled else ("failed" if error else "done")
        message = "行业信息补齐完成" if status == "done" else (
            "行业信息补齐任务已取消" if status == "cancelled" else f"行业信息补齐失败：{error}"
        )
        updates: dict[str, Any] = {
            "status": status,
            "stage": status,
            "message": message,
            "result_json": json.dumps(result or {"error": str(error)}, ensure_ascii=False, default=str),
            "finished_at": _now(),
        }
        if status == "done":
            updates["percent"] = 100
        _set_task(db, task_id, **updates)
    finally:
        db.close()
