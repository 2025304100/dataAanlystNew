...
def _run_backtest_decision_driven(
    ...
    *,
    ...
    try:
        ...
        # P2 BFG：持久化 backtest_runs.config_hash / filter_config_json / production_fidelity
        # 与 run_backtest_precheck 默认 filter_config=None 路径的 BacktestFilterConfig() 对齐
        try:
            from dataclasses import asdict as _asdict_bfg
            from app.services.backtest_filters.config import (
                BacktestFilterConfig as _BfgCfg,
                compute_config_hash as _bfg_hash,
                validate_production_fidelity as _bfg_validate,
            )
            _bfg_cfg_obj = _BfgCfg()
            run.config_hash = _bfg_hash(_bfg_cfg_obj)
            run.filter_config_json = json.dumps(_asdict_bfg(_bfg_cfg_obj), ensure_ascii=False)
            _bfg_ok, _bfg_reason = _bfg_validate(_bfg_cfg_obj)
            run.production_fidelity = int(bool(_bfg_ok))
            run.non_fidelity_reason = _bfg_reason or None
        except Exception:
            # BFG 配置持久化是非关键路径，不得中断主回测链路
            pass
        db.flush()
        db.commit()
        return run
