...
def _industry_map_from_frame(frame: pd.DataFrame, *, strict: bool = False) -> dict[str, str]:
    """从 AkShare 股票列表解析行业映射。strict 用于历史补齐的失败识别。"""
    if frame is None or frame.empty:
        if strict:
            raise RuntimeError("A股股票行业数据源返回空数据")
        return {}
    code_key = next((key for key in ("代码", "code", "symbol") if key in frame.columns), None)
    industry_key = next((key for key in ("所属行业", "行业", "industry") if key in frame.columns), None)
    if not code_key or not industry_key:
        if strict:
            raise RuntimeError(f"A股股票行业数据源缺少代码或行业列：{list(frame.columns)}")
        logger.warning("universe cn-stock industry source missing columns: %s", list(frame.columns))
        return {}
    result: dict[str, str] = {}
    for row in frame[[code_key, industry_key]].to_dict("records"):
        raw_code = row.get(code_key)
        raw_industry = row.get(industry_key)
        ...
        if code.isdigit() and industry and industry.lower() != "nan":
            result[code] = industry[:64]
    return result


def _fetch_cn_stock_industry_map(db: Session, *, strict: bool = False) -> dict[str, str]:
    """批量拉取 A 股股票行业，返回标准化代码到行业名称的映射。"""
    try:
        with quiet_akshare_output():
        ...
        logger.warning("universe cn-stock industry source failed: %s", exc)
        if strict:
            raise RuntimeError(f"A股股票行业数据源请求失败：{exc}") from exc
        return {}
    return _industry_map_from_frame(frame, strict=strict)
...
def refresh_universe_symbols(scope: str, db: Session) -> dict:
    ...
    if scope == "cn-stock":
        has_code_column = any(key in frame.columns for key in ("代码", "code", "symbol"))
        has_industry_column = any(key in frame.columns for key in ("所属行业", "行业", "industry"))
        # stock_info_a_code_name 没有行业列时才请求 spot_em；避免重复请求。
        industry_map = (
            _industry_map_from_frame(frame)
            if has_code_column and has_industry_column
            else _fetch_cn_stock_industry_map(db)
        )
    seen = 0
    created = 0
    skipped_suspended = 0
...
def backfill_universe_stock_industries(
    *,
    progress_callback: Callable[[int, int, int, int], None] | None = None,
    is_cancelled: Callable[[], bool] | None = None,
) -> dict[str, int]:
    """一次性补齐已有 A 股股票的行业字段，不触碰 K 线和同步状态。"""
    db = SessionLocal()
    try:
        symbols = db.execute(
        ...
        if is_cancelled and is_cancelled():
            return {"total": total, "processed": 0, "updated": 0, "skipped": 0, "failed": 0}
        if not symbols:
            return {"total": 0, "processed": 0, "updated": 0, "skipped": 0, "failed": 0}

        # 历史补齐必须能明确区分“没有可更新行业”和“数据源失败”。
        industry_map = _fetch_cn_stock_industry_map(db, strict=True)
        processed = updated = skipped = failed = 0
        for item in symbols:
            if is_cancelled and is_cancelled():
                break
            try:
                industry = industry_map.get(item.symbol)
                if industry and not item.industry:
            ...
            if processed % 50 == 0 or processed == total:
                db.commit()
                if progress_callback:
                    progress_callback(processed, total, updated, failed)
        db.commit()
        return {"total": total, "processed": processed, "updated": updated, "skipped": skipped, "failed": failed}
    finally:
        db.close()
