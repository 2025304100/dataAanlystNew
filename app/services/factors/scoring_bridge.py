...
def materialize_factor_scores(
    ...
    *,
    ...
    coefficients = {
        member.factor_code: float(member.coefficient) for member in model_members
    }
    normalized_weights = {
        member.factor_code: float(member.normalized_weight) for member in model_members
    }
    # WP7-03: 从 model.weights 动态派生特征列表（替代静态 FEATURE_CODES）
    feature_codes = tuple(
        member.factor_code for member in
        sorted(model_members, key=lambda member: member.factor_code)
    )
    ...
    member_versions_snapshot = {
        member.factor_code: {
            'version': member.factor_version,
            'coefficient': float(member.coefficient),
            'normalized_weight': float(member.normalized_weight),
        }
