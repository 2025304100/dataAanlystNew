...
⚠️ 注册**四处**联动（新增算子缺一不可）：
   1. `factor_compiler.FUNCTION_CATALOG`      编译期函数白名单
   2. `factor_compiler.FUNCTION_DIRECTION`    计算方向标注（决定求值轴）
   3. `formula_catalog._FUNCTION_META`        公式目录元数据（**硬索引**，漏了 KeyError）
   4. `indicator_ast_sandbox.ALLOWED_FUNCS`   单标的沙箱白名单

   第 4 处是**登记但不可求值**：本模块是面板级算子，单标的沙箱无法求值，
   故沙箱在构造期即显式拒绝（不是静默返回 None），见该模块 docstring。

⚠️ 求值顺序：嵌套时**从内到外**逐层计算（如 `cs_rank(sma(close, 20))` 先算 sma）。
   可用 `factor_compiler.collect_directional_calls(ast)` 取到内层优先的算子序列。
"""
