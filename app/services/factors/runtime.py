...
def activate_factor_model(
    ...
    *,
    ...
    # Do not allow a model with an empty or corrupt coefficient snapshot into
    # runtime.  Such models previously passed the status check and only failed
    # later when the scoring bridge attempted to materialize scores.
    model_members = list(db.execute(
        select(FactorModelMember).where(FactorModelMember.model_run_id == model.id)
    ).scalars())
    if not model_members:
        raise ValueError('validated model has no factor coefficients; retrain the model before activation')
    import math
    if any(
        not math.isfinite(float(member.coefficient))
        or not math.isfinite(float(member.normalized_weight))
        for member in model_members
    ):
        raise ValueError('validated model has invalid factor coefficients; retrain the model before activation')
    state = db.execute(
