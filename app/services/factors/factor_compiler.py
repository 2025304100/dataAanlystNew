...
from app.services.factors.dsl.dialect import (  # noqa: E402
    DIALECT_ERROR_CODE,
    DialectError,
    normalize_dialect,
)
...
ERROR_CODES = frozenset({
    ...
    # M2/T05 新增：方言归一失败（典型：delta() 无法判定行情/财报）
    "dialect_normalization_failed",
})

# 与 dialect 模块共用同一个字面量，避免两处各写一份而漂移
assert DIALECT_ERROR_CODE in ERROR_CODES, "dialect 错误码未登记进 ERROR_CODES"
...
class FunctionSpec:
    ...
    name: str
    # "math" | "rolling" | "conditional"（既有）
    # | "timeseries"（M2/T04 新增）| "cross_section"（M2/T03 新增）
    # ⚠️ 取值必须与 FUNCTION_DIRECTION 的方向常量**拼写一致**（timeseries / cross_section），
    #    不要写成 time_series —— 两套拼写会让按 category 分派的下游静默漏掉分支。
    category: str
    min_args: int
    max_args: int
    description: str
...
FIELD_CATALOG: dict[str, FieldSpec] = {
    ...
    # ⚠️ prev_close 是**可用的虚字段**，不是「虚注册」——不要删！
    #    它没有物理列，但 `FactorExecutor._select_field_expressions`（factor_executor.py:679-681）
    #    会把它投影成 `LAG(close) OVER (PARTITION BY symbol ORDER BY trade_date) AS prev_close`，
    #    两个取数点（:891 / :1352）都走这条投影，因此 `close / prev_close - 1` 可正常求值。
    #    2026-09-16 在真实数仓端到端实测：000001 2026-07-21~24 的 prev_close = 10.98/10.84/10.98/11.08，
    #    与上一交易日收盘价逐条相等；既有测试 test_preview_derives_prev_close_without_physical_column 亦为绿灯。
    #    历史结论「撤销 prev_close 注册」基于「无派生实现」的错误前提，已撤销（SD-v2.0 §12.3 D-D）。
    "prev_close": FieldSpec(
        "prev_close", "raw_daily_bars", "float", "前收盘价（由 close 的 LAG 派生，无物理列）", layer="A"
    ),
    ...
    # A 层估值扩展（raw_valuation_snapshots，point-in-time）—— M1/T06 注册，零采集。
    # ⚠️ 三列的**数据现状不同**（2026-09-16 实测 raw_valuation_snapshots 3,079,528 行）：
    #     total_market_cap / circulating_market_cap 非空 100%；
    #     dividend_yield 非空 **0 行（全 NULL）** —— 列存在但从未采集。
    #     因此 dividend_yield 不写死 data_mode=blocked（那会在补数据后仍永久阻断），
    #     而是交给 formula_catalog 的覆盖率检测自动判 blocked，补数后自动转为可用。
    "dividend_yield": FieldSpec(
        "dividend_yield", "raw_valuation_snapshots", "float", "股息率",
        point_in_time=True, layer="A",
    ),
    "total_market_cap": FieldSpec(
        "total_market_cap", "raw_valuation_snapshots", "float", "总市值",
        point_in_time=True, layer="A",
    ),
    "circulating_market_cap": FieldSpec(
        "circulating_market_cap", "raw_valuation_snapshots", "float", "流通市值",
        point_in_time=True, layer="A",
    ),
...
FUNCTION_CATALOG: dict[str, FunctionSpec] = {
    ...
    # ── 截面算子族（M2 / T03，设计文档 §6.2，需求 §6.0）──────────
    # 语义：按 trade_date 分组、横截面（同日全标的）向量化求值。
    # 与后处理配置项 rank/zscore **语义不同、并存**：后处理只作用于最终因子值一次
    # （factor_executor.apply_postprocess），本组算子作用于公式求值阶段，因此可以
    # 表达 `cs_rank(A) - cs_rank(B)` 这类后处理做不到的式子。
    #
    # ⚠️ cs_quantile / cs_winsorize 复用 window_arg_index=1：
    #    它们的第 2 个参数是「分位组数 / MAD 倍数」，同样必须是常量正整数，
    #    复用既有校验路径（常量 → 正数 → ≤ max_lookback）即可，无需新增机制。
    #    副作用：该常量会被 _collect_dependencies 计入 max_lookback（无害，见 L840）。
    #    注意 `method="quantile"` 的比值语义无法从公式触发（DSL 禁止关键字参数），
    #    公式内 `cs_winsorize(x, n)` 只走 MAD 分支。
    "cs_rank": FunctionSpec(
        "cs_rank", "cross_section", 1, 1, "横截面百分位排名"
    ),
    "cs_zscore": FunctionSpec(
        "cs_zscore", "cross_section", 1, 1, "横截面标准化"
    ),
    "cs_demean": FunctionSpec(
        "cs_demean", "cross_section", 1, 1, "横截面去均值"
    ),
    "cs_scale": FunctionSpec(
        "cs_scale", "cross_section", 1, 1, "横截面缩放到单位和"
    ),
    "cs_quantile": FunctionSpec(
        "cs_quantile", "cross_section", 2, 2, "横截面分位组编号", window_arg_index=1
    ),
    "cs_winsorize": FunctionSpec(
        "cs_winsorize", "cross_section", 2, 2, "横截面去极值（MAD）", window_arg_index=1
    ),
    # ── 时序算子族（M2 / T04，设计文档 §6.2，需求 §6.0）──────────
    # 语义：按**标的**沿时间轴位移/滚动（与截面算子按 trade_date 分组相反）。
    # ⚠️ `n` 的单位是两个 delta 算子**唯一的差别**，必须显式拆算子而不是靠 AST 猜字段类型：
    #      ts_delta_bars    n = **交易日**  → ts_delta_bars(close, 20)  过去 20 个交易日
    #      ts_delta_periods n = **报告期**  → ts_delta_periods(roe_ttm, 4) 过去 4 个季度·
    #    机器可读的单位表见 `app/services/factors/dsl/time_series.py::TS_N_UNIT`。
    # ⚠️ ts_atr 用 close-to-close 口径（`TR = |close - ref(close,1)|` 的 n 期均值），
    #    **不是**含 high/low 的经典 ATR —— 原因：执行器按依赖字段裁剪 SELECT，
    #    偷偷取 high/low 会因列缺失静默产出全 NaN（见 time_series.py 模块 docstring）。
    "ts_delta_bars": FunctionSpec(
        "ts_delta_bars", "timeseries", 2, 2,
        "行情字段 N 个交易日差分", window_arg_index=1,
    ),
    "ts_delta_periods": FunctionSpec(
        "ts_delta_periods", "timeseries", 2, 2,
        "财报字段 N 个报告期差分", window_arg_index=1,
    ),
    "ts_atr": FunctionSpec(
        "ts_atr", "timeseries", 2, 2,
        "平均真实波幅（close-to-close）", window_arg_index=1,
    ),
}
...
# ══════════════════════════════════════════════════════════
#
# 「计算方向」= 算子要在**哪个轴上**聚合，与 `ExecutionPlan.direction`
# （因子方向：higher_better / lower_better）是**两个不同的概念**，不要混用。
#
#   timeseries   → 沿时间轴、对**单个标的**滚动求值（sma / ref / ts_delta_* …）
#   cross_section → 在**同一交易日**、对全截面标的求值（cs_* 算子族）
#
# 未登记的函数（math / rolling / conditional）为**逐元素**求值，无方向。
# 嵌套时必须**从内到外**逐层计算：内层算子先出结果，外层才能按截面分组
# （见 `collect_directional_calls`）。

DIRECTION_TIMESERIES: str = "timeseries"
DIRECTION_CROSS_SECTION: str = "cross_section"

FunctionDirection = Literal["timeseries", "cross_section"]

#: 算子名 → 计算方向。仅登记**需要特殊求值轴**的算子；逐元素算子不登记。
#: T03 登记截面族；T04 追加时序族。
FUNCTION_DIRECTION: dict[str, FunctionDirection] = {
    "cs_rank": DIRECTION_CROSS_SECTION,
    "cs_zscore": DIRECTION_CROSS_SECTION,
    "cs_demean": DIRECTION_CROSS_SECTION,
    "cs_scale": DIRECTION_CROSS_SECTION,
    "cs_quantile": DIRECTION_CROSS_SECTION,
    "cs_winsorize": DIRECTION_CROSS_SECTION,
    "ts_delta_bars": DIRECTION_TIMESERIES,
    "ts_delta_periods": DIRECTION_TIMESERIES,
    "ts_atr": DIRECTION_TIMESERIES,
}


def get_function_direction(name: str) -> FunctionDirection | None:
    """返回算子的计算方向；逐元素算子返回 None。"""
    return FUNCTION_DIRECTION.get(name)


def collect_directional_calls(
    node: ast.AST,
) -> list[tuple[str, FunctionDirection]]:
    """按**从内到外**顺序返回公式中所有带计算方向的算子调用。

    对应设计文档 §6.2「AST 节点标注计算方向，嵌套时从内到外逐层计算」。
    供 `factor_executor` 的截面 / 时序求值分支决定求值顺序：内层算子必须先算出
    结果，外层才能对它做截面分组（如 `cs_rank(sma(close, 20))`）。

    Returns:
        [(算子名, 方向), ...]，后序遍历（最内层在前）。同一层按源码出现顺序。

    ⚠️ 本函数**不参与** `ExecutionPlan.to_canonical_json()` 与 `content_hash`。
       `content_hash` 是 `factor_versions.execution_plan_hash` 的去重键，改动其输入
       会让所有既有因子版本的哈希失效、去重退化为重复建版本（见 factor_registry）。
       因此方向标注只作为**运行时查询**，不写进执行计划。
    """
    out: list[tuple[str, FunctionDirection]] = []

    def visit(current: ast.AST) -> None:
        for child in ast.iter_child_nodes(current):
            visit(child)  # 先子后父 = 从内到外
        if isinstance(current, ast.Call) and isinstance(current.func, ast.Name):
            direction = FUNCTION_DIRECTION.get(current.func.id)
            if direction is not None:
                out.append((current.func.id, direction))
...
class FactorCompiler:
    ...
    def compile(
        ...
        *,
        ...
        # 2a. 方言归一（M2/T05）。**仅在公式内生效**：后处理配置不经此路径。
        #     位置在 `_normalize_dsl_surface` 之前 —— 方言改写可能产生 `iif` 之外的
        #     新算子名，两者是串联的两道预处理。
        try:
            dialect_formula = normalize_dialect(source_formula)
        except DialectError as exc:
            return CompilationResult(
                success=False,
                errors=[_error_at_token(
                    source=source_formula,
                    error_code=DIALECT_ERROR_CODE,
                    message=exc.message,
                    detail=exc.detail,
                    token=exc.token,
                )],
            )
        normalized_formula = _normalize_dsl_surface(dialect_formula)
        try:
