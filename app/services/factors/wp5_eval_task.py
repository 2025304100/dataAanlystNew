...
def _cancelled(task_id: str) -> bool:
    ...
    try:
        ...
        return task is not None and (
            ...
            or int(getattr(task, "cancel_requested", 0) or 0) == 1
        )
    finally:
        cancel_db.close()
...
def _finalize_cancelled(task_id: str) -> None:
    """Promote a cooperative cancel request to the terminal task state."""
    SessionLocal = get_session_local()
    db = SessionLocal()
    try:
...
def preflight_factor_evaluation(
    *,
    ...
    # The compiler treats a bare column expression (for example ``close``)
    # as an already-normalized execution node and may omit it from fields.
    # Preserve the dependency gate for that common case so missing columns
    # cannot incorrectly produce an overall pass.
    if not needed_fields and isinstance(formula, str):
        import re as _re
        tokens = _re.findall(r"[A-Za-z_][A-Za-z0-9_]*", formula)
        ignored = {
            "and", "or", "not", "if", "else", "True", "False", "None",
            "abs", "log", "sqrt", "rank", "zscore", "mean", "std", "min", "max",
        }
        needed_fields = sorted({t for t in tokens if t not in ignored})
    ...
    missing = sorted({
        ...
        and not (
            derived_fields.get(field_name)
            and all(
                dependency in available_all
                for dependency in derived_fields[field_name]
            )
        )
    })
...
def create_evaluation_task(
    *,
    ...
    # A previous failed/cancelled attempt must not poison the stable business
    # idempotency key forever; allow a fresh retry while still reusing active
    # and successful tasks above.
    retry_failed = False
    for t in existing:
        if t.status not in {"failed", "cancelled"} or not t.payload_json:
            continue
        try:
            retry_failed = compute_payload_fingerprint(json.loads(t.payload_json)) == fingerprint
    ...
    task = create_async_task(
        ...
        force_new=force_new or retry_failed,
    )
    d = task.model_dump()
    d["fingerprint"] = fingerprint
    if "task_id" not in d and "id" in d:
        d["task_id"] = d["id"]
    # The create contract returns an accepted queued task. A worker outcome is
    # available from the task-status endpoint and must not race this response.
    d["status"] = "queued"
    d["stage"] = "queued"
    d["percent"] = 0
    d["message"] = "Task created"
    # Return the queued snapshot before scheduling the worker. This avoids a
    # race where a very fast validation failure changes the response to
    # ``failed`` before the create endpoint has returned.
...
def _run_evaluation_worker(task_id: str) -> None:
    ...
    try:
        ...
        # Keep scalar values independent of the ORM instance. The background
        # worker spans commits and request/test cleanup can expire or delete
        # the version row before later stages read it.
        version_formula_expr = str(version.formula_expr or "close")
        version_params_json = version.params_json
        version_postprocess_json = version.postprocess_json
        version_direction = version.direction
        version_id = version.id

        _set_task(db, task_id, stage="loading", percent=10,
                  message=f"已加载版本 v{version.version}（id={version_id}）")
        ...
        _set_task(db, task_id, stage="compiling", percent=25,
                  message="编译因子公式")
        params = json.loads(version_params_json) if version_params_json else {}
        postprocess = json.loads(version_postprocess_json) if version_postprocess_json else None

        compile_result = compile_formula(
            formula=version_formula_expr,
            params=params,
            postprocess=postprocess,
            direction=version_direction or factor.direction or "higher_better",
            strict_fields=True,
        )
        if not compile_result.is_valid:
            ...
            _set_task(db, task_id, status="failed", stage="failed",
                      ...
                      result_json=json.dumps({
                          ...
                          "formula_expr": version_formula_expr,
        ...
        if run is not None:
            ...
            # TD3：metrics_json 统一清洗（stress_test 子 dict 嵌套数值也在内）+ fail-fast
            run.metrics_json = json.dumps(
                clean_json_tree(existing_metrics), ensure_ascii=False, default=str, allow_nan=False
            )
            db.flush()

        # 8. 完成（只有压力通过才用 phase1 的 passed/rejected，否则一律 warn）
        # Cancellation is terminal and wins over a worker that finishes at the
        # same time as the cancel request.
        if _cancelled(task_id):
            _finalize_cancelled(task_id)
            return
        final_status = "done"
        ...
        try:
            existing = db.get(AsyncTaskRecord, task_id)
            # User cancellation wins over a concurrent worker exception. This
            # keeps the task state truthful even when validation fails while
            # the cancellation request is being processed.
            if existing is not None and (
                existing.status == "cancelled"
                or int(getattr(existing, "cancel_requested", 0) or 0) == 1
            ):
                if existing.status != "cancelled":
                    existing.status = "cancelled"
                    existing.stage = "cancelled"
                    existing.message = "Task cancelled by user"
                    existing.finished_at = _now()
                    existing.is_terminal_locked = 1
                    db.commit()
                return
        except Exception:  # noqa: BLE001
            try:
...
def get_evaluation_task_heartbeat(task_id: str) -> dict | None:
    """Read-only lightweight status for long-running task heartbeats."""
    from app.services.async_tasks import get_async_task_heartbeat
    return get_async_task_heartbeat(task_id)
