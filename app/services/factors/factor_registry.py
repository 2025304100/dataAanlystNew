...
def create_factor_version(
    ...
    *,
    ...
    # 已被引用的版本仍然保持内容不可变，但允许创建新的版本。
    # is_latest 只是路由指针，切换它不会修改历史版本内容；此前将
    # “被引用”误判为“禁止创建新版本”，导致公式编辑器无法迭代。
...
def get_factor_references(
    ...
    model_run_count = int(
        db.execute(
            select(func.count())
            .select_from(FactorModelMember)
            .where(
                FactorModelMember.factor_code == factor.code,
                FactorModelMember.factor_version == version.version,
            )
        ).scalar_one()
    )
