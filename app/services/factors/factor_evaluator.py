...
def finalize_evaluation_run(
    ...
    *,
    ...
    # TD3：NaN/±Inf → null（D-F/R33 口径，不许归 0）+ allow_nan=False fail-fast 双保险
    run.metrics_json = json.dumps(
        clean_json_tree(metrics), ensure_ascii=False, default=str, allow_nan=False
    )
