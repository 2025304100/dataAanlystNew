...
class TimePerturbationResult:
    ...
    verdict: str  # stable / unstable / insufficient
...
def perturb_time_segments(
    *,
    ...
    if len(common_dates) < segment_size * 2:
        # 数据本身可能有效，但不足以形成两个完整时间段；这不是
        # “时间不稳定”，应由门禁作为样本不足单独提示，而不是误报失败。
        return TimePerturbationResult(segments=[], ic_stability=1.0, verdict="insufficient")
    ...
    if len(segments) < 2:
        return TimePerturbationResult(segments=segments, ic_stability=1.0, verdict="insufficient")
