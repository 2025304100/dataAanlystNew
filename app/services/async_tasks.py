...
def get_async_task_heartbeat(task_id: str) -> dict | None:
    """Read-only lightweight task status for long-running heartbeat polling.

    Unlike ``get_async_task`` this endpoint never runs patrol/expiry writes,
    so frequent status reads cannot contend with the worker's transaction.
    """
    SessionLocal = _pick_session_factory(True)
    db = SessionLocal()
    try:
        task = db.get(AsyncTaskRecord, task_id)
        if task is None:
