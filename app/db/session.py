...
from app.core import text_charset as _text_charset  # noqa: F401

# C1 裁决（TD6，2026-09-18）：全库 utf8mb3，import 即对全局 Session 挂
# before_flush 守卫——4 字节字符（emoji/CJK 扩展区）在写库前被拦截为
# ValueError（带类名.字段名），替代 DB 层的 1366 Incorrect string value。
_text_charset.install_bmp_text_guard()
...
def reset_control_plane_cache():
    """失效控制平面缓存并返回旧 engine（TD2 收尾裁决新增的公开失效接口）。

    场景：进程内 DatabaseManager 的 engine 被重新指向（集成测试每用例改绑
    tmp 库、或运维热切换 DATABASE_URL）时，模块级缓存仍持有按旧 URL 建的
    engine——旧库文件删除后控制面会持续轮询死库（任务永远 queued、cancel
    404、状态匹配 0 条）。调用本函数后，下一次 get_control_session_local()
    会基于当前 dm.engine.url 重建。

    契约：持 _cp_lock 置空缓存、锁外 dispose（dispose 可能阻塞，不得与
    _cp_lock 同持），返回被失效的旧 engine（可为 None）供日志/断言。
    """
    global _cp_engine, _cp_factory
    with _cp_lock:
        stale = _cp_engine
