...
# 因子挖掘（Factor Mining）：候选池 / 批次 / 候选 / 双锁 / 评估
api_router.include_router(factor_mining.router, tags=['factor-mining'])
# 候选池（M3/T08）：池 CRUD + 成员软删除 + 审计
api_router.include_router(mining_candidate_pool.router, tags=['factor-mining'])
# 数据中心（M1a/T15）：历史行情镜像（MySQL→DuckDB，分块续跑）
api_router.include_router(data_mirror.router, tags=['data-mirror'])
# 挖掘向导（M15/T14）：字段异步校验（分片 + 断点续跑 + 24h 有效期）
api_router.include_router(factor_mining_wizard.router, tags=['factor-mining'])
