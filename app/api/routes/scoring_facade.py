...
class CreateScoringTaskRequest(BaseModel):
    ...
    Task 9 (3) 三处 factor_set_id 一致写入：
      ① payload.factor_set_id         — 顶层字段（写入 async_tasks.payload_json → pipeline_runs.payload）
      ② hyperparameters.factor_set_id — 训练超参数字典（写入 FactorModelRun.hyperparameters_json）
      ③ factor_scope.factor_set_id    — 因子作用域字典（写入 factor_audit_logs.attributes_json）
    三处值严格相等；若不一致 → HTTP 400 PIPELINE_INCONSISTENT_FACTOR_SET。
    """
    ...
    # Task 9 (3) 三处一致写入 — ② hyperparameters ③ factor_scope
    hyperparameters: dict | None = Field(
        default=None,
        description="训练超参数；train=true 时若含 factor_set_id 必须等于顶层 factor_set_id。",
    )
    factor_scope: dict | None = Field(
        default=None,
        description="因子作用域；train=true 时若含 factor_set_id 必须等于顶层 factor_set_id。",
    )
...
def api_create_scoring_task(req: CreateScoringTaskRequest) -> dict:
    ...
    Task 9 (3) 三处 factor_set_id 一致写入（仅 train=true 时强制校验）：
      ① payload.factor_set_id == ② hyperparameters.factor_set_id == ③ factor_scope.factor_set_id
    若三处有多处指定且值不一致 → HTTP 400 PIPELINE_INCONSISTENT_FACTOR_SET（7 要素）。
    """
    from app.schemas.errors import factor_structured_error

    # ── Task 9 (3) 三处一致校验（前门检查，仅 train=true） ──
    if req.train_model:
        fs_top = (
            str(req.factor_set_id).strip()
            if req.factor_set_id is not None and str(req.factor_set_id).strip()
            else None
        )
        hp = req.hyperparameters or {}
        fs_hp_raw = hp.get("factor_set_id") if isinstance(hp, dict) else None
        fs_hp = (
            str(fs_hp_raw).strip()
            if fs_hp_raw is not None and str(fs_hp_raw).strip()
            else None
        )
        sc = req.factor_scope or {}
        fs_sc_raw = sc.get("factor_set_id") if isinstance(sc, dict) else None
        fs_sc = (
            str(fs_sc_raw).strip()
            if fs_sc_raw is not None and str(fs_sc_raw).strip()
            else None
        )
        # 收集所有非空值
        specified: list[str] = []
        if fs_top:
            specified.append(fs_top)
        if fs_hp:
            specified.append(fs_hp)
        if fs_sc:
            specified.append(fs_sc)
        # 如果有多个不同值 → 不一致错误
        unique_values = set(specified)
        if len(unique_values) > 1:
            err = factor_structured_error(
                "PIPELINE_INCONSISTENT_FACTOR_SET",
                title_zh="三处 factor_set_id 写入不一致",
                detail_zh=(
                    f"train=true 时三处必须相同：payload.factor_set_id={fs_top!r}，"
                    f"hyperparameters.factor_set_id={fs_hp!r}，"
                    f"factor_scope.factor_set_id={fs_sc!r}。请统一后重试。"
                ),
                fix_link="/settings/pipeline",
                retryable=False,
                extras={
                    "payload.factor_set_id": fs_top,
                    "hyperparameters.factor_set_id": fs_hp,
                    "factor_scope.factor_set_id": fs_sc,
                },
            )
            raise HTTPException(status_code=400, detail=err.to_dict())
        # 若前门指定了一致值（比如只在 hyperparameters 里指定），把顶层 factor_set_id 统一
        if len(unique_values) == 1:
            canonical_fsid = next(iter(unique_values))
            if fs_top != canonical_fsid:
                req.factor_set_id = canonical_fsid
    ...
    try:
        task = create_scoring_task(
            ...
            factor_scope=req.factor_scope,
        )
    except FactorSevenError as fse:
        # Task 9 (1)(2) 抛出 7 要素结构化错误 → HTTP 400 detail=完整 7 字段 dict
        raise HTTPException(status_code=400, detail=fse.to_dict()) from fse
    except ValueError as exc:
...
@router.get("/scoring/models/{model_id}/relations", response_model=dict)
def api_get_model_relations(model_id: str) -> dict:
    """Unified model → set → members → weights DTO (Task 5 / FR-5).

    Shared by FactorModelPage.tsx and FactorModelSettings.tsx — frontends
    MUST NOT re-parse hyperparameters_json / weights_json themselves.
    Returns 404 if the model_id does not exist.

    When the model has no factor_set_id reference (historical run), the
    response carries ``unbound_reason`` with AC-7's full Chinese text and
    empty relation arrays (factors=[], n_members=None, set id/name=None).
    """
    relations: ModelRelationsDTO | None = factor_model_with_relations(model_id)
    if relations is None:
        raise HTTPException(status_code=404, detail=f"Model run {model_id} not found")
    return _dc2dict(relations)
...
class RenameScoringModelRequest(BaseModel):
...
class CloneScoringFactorSetRequest(BaseModel):
    """P1.2: 复制集合请求（AC-18 action=clone_set，clone后新集合status=draft可重新编辑）。"""
    new_factorset_id: str = Field(..., min_length=1, max_length=64)
...
@router.patch("/scoring/models/{model_id}/alias", response_model=dict)
def api_rename_scoring_model(model_id: str, req: RenameScoringModelRequest):
    """Update a human-readable model alias while preserving the immutable ID."""
    from app.db.session import get_session_local
    from app.models.factor_model import FactorModelRun
    import json
    db = get_session_local()()
    try:
        model = db.get(FactorModelRun, model_id)
        if model is None:
            raise HTTPException(status_code=404, detail="模型不存在")
        hp = {}
        try:
            hp = json.loads(model.hyperparameters_json or "{}")
        except (TypeError, json.JSONDecodeError):
            hp = {}
        hp["display_alias"] = req.alias.strip()
        model.hyperparameters_json = json.dumps(hp, ensure_ascii=False)
        db.commit()
        return {"ok": True, "model_id": model_id, "alias": hp["display_alias"]}
    finally:
        db.close()
...
@router.post("/scoring/factorsets/{factorset_id}/clone", response_model=dict)
def api_clone_scoring_factorset(factorset_id: str, req: CloneScoringFactorSetRequest):
    """P1.2 薄封复制集合。成功后生成 factor_audit_logs action=clone_set。"""
    try:
        return clone_scoring_factor_set(
            factorset_id,
            new_factorset_id=req.new_factorset_id,
...
@router.post("/scoring/evaluations/precheck", summary="因子评价预检（BFG 过滤治理）")
def scoring_factor_evaluation_precheck(
    payload: FactorEvaluationPrecheckRequest,
    db: Session = Depends(get_db),
):
    from app.services.factors.__facade__ import run_factor_eval_precheck

    return run_factor_eval_precheck(
        db,
        ...
        minimum_trade_days=payload.minimum_trade_days,
    )
