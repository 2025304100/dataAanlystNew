...
class AiConfigFields(BaseModel):
    ...
    @classmethod
    def normalize_model_text(cls, value):
        # 老版本前端的 tags 控件可能提交 ["model-id"]；接受并转换，
        # 同时保持 API 对外的规范类型仍为单个字符串。
...
def _normalized_config(values: dict, *, saved: dict | None = None) -> dict:
    ...
    # 兼容旧版 tags 下拉控件提交的 string[]：模型名称本质上只能是单个文本。
    raw_model = cfg.get("model", "")
    if isinstance(raw_model, (list, tuple)):
        raw_model = next((item for item in raw_model if str(item).strip()), "")
    cfg["model"] = str(raw_model).strip()
...
def _extract_ai_reply(data: dict) -> str:
    ...
    return str(data.get("response") or data.get("output_text") or "")
...
def ai_chat_stream(req: AiChatRequest):
    ...
    def event_stream():
        ...
        try:
            ...
            # 部分兼容 OpenAI 的网关会忽略 stream=true，直接返回一次性
            # JSON，因而不会产生可解析的 delta。自动回退一次非流式请求，
            # 避免前端只看到“未返回有效回复”。
            if not raw_reply:
                # Retry non-streaming twice: transient empty completions are
                # observed with gateways that spend the first attempt on
                # hidden reasoning tokens.
                fallback_payload = _build_chat_payload(cfg, messages, stream=False)
                fallback_timeout = max(float(cfg["timeout_seconds"]), 120.0)
                with httpx.Client(timeout=fallback_timeout) as fallback_client:
                    for attempt in range(2):
                        fallback_resp = fallback_client.post(
                            endpoint,
                            json=fallback_payload,
                            headers=headers,
                        )
                        logger.info(
                            "AI chat fallback attempt=%s status=%s body_bytes=%s",
                            attempt + 1, fallback_resp.status_code, len(fallback_resp.content),
                        )
                        if fallback_resp.status_code < 400:
                            raw_reply = _extract_ai_reply(fallback_resp.json()) or ""
                        if raw_reply:
                            break
