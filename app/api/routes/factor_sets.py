...
def post_add_member(factor_set_id: str, payload: FactorSetMemberCreate):
    ...
    with SessionLocal() as db:
        try:
            # The UI deliberately uses code + human-facing version number.
            # Resolve them server-side to avoid exposing internal database IDs.
            if payload.factor_id is None or payload.factor_version_id is None:
                from app.models.factor import Factor
                from app.models.factor_model import FactorVersion

                factor = db.execute(
                    select(Factor).where(Factor.code == payload.factor_code)
                ).scalar_one_or_none()
                if factor is None:
                    raise FactorSetError("factor_not_found", f"Factor {payload.factor_code} 不存在")
                version = db.execute(
                    select(FactorVersion).where(
                        FactorVersion.factor_id == factor.id,
                        FactorVersion.version == payload.factor_version,
                    )
                ).scalar_one_or_none()
                if version is None:
                    raise FactorSetError(
                        "version_not_found",
                        f"Factor {payload.factor_code} 的版本 v{payload.factor_version} 不存在",
                    )
                payload.factor_id = factor.id
                payload.factor_version_id = version.id
