...
@router.post("/universe/industry-backfill", response_model=AsyncTaskRead)
def start_industry_backfill():
    """一次性补齐已有 A 股股票的行业字段，不同步或修改 K 线。"""
    return universe_sync_task.start_universe_industry_backfill()


@router.get("/universe/industry-backfill/status", response_model=AsyncTaskRead | None)
def get_industry_backfill_status():
    return universe_sync_task.get_latest_universe_industry_backfill_task()


@router.post("/universe/industry-backfill/cancel", response_model=AsyncTaskRead | None)
def cancel_industry_backfill():
    latest = universe_sync_task.get_latest_universe_industry_backfill_task()
    if latest is None:
        raise HTTPException(status_code=404, detail="无行业字段补齐任务")
    if latest.status not in ("queued", "running"):
        raise HTTPException(status_code=400, detail=f"任务已处于终态：{latest.status}")
    result = universe_sync_task.cancel_universe_industry_backfill_task(latest.id)
    if result is None:
        raise HTTPException(status_code=404, detail="任务不存在")
    return result
