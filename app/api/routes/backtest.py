...
# ---------------------------------------------------------------------------
# Task 12.2: 7 要素结构化错误辅助函数
# ---------------------------------------------------------------------------
_RETRYABLE_DB_KEYWORDS = (
    "timeout", "lock", "deadlock", "busy", "temporarily", "503", "connection",
    "lost", "pool", "operationalerror", "interfaceerror",
)
...
def _is_db_retryable(exc: BaseException) -> bool:
    """简单关键词启发式：DB 临时异常/超时 => retryable=True。"""
    text = f"{type(exc).__name__} {exc}".lower()
    return any(k in text for k in _RETRYABLE_DB_KEYWORDS)


# Task 12.1: data_cutoff 查询（复用 bfg_precheck_service 的计算函数，失败回退 None）
def _safe_compute_data_cutoff(db: Session):
    try:
        from app.services.bfg_precheck_service import compute_data_cutoff
        return compute_data_cutoff(db)
...
def _map_status_to_stage(status: str | None) -> str:
...
def _enrich_backtest_result_contract(
    ...
    *,
    ...
    # T11 11.2: 投影 stage / progress_pct / updated_at / error_code / retryable
...
def create_portfolio_backtest_run(payload: PortfolioBacktestRequest, request: Request, db: Session = Depends(get_db), x_frontend_session_id: str | None = Header(default=None, alias="X-Frontend-Session-Id")):
    """P2-2：组合整体回测。
    ...
    # Task1 fail-closed: re-run precheck before real execution; block on any blocker or exception.
    try:
        ...
        pre_req = BacktestPrecheckRequest(
            portfolio_id=payload.portfolio_id,
            symbol_ids=[],
            start_date=payload.start_date,
            end_date=payload.end_date,
        ...
        raise HTTPException(
            ...
            detail=_build_structured_error(
                ...
                title_zh="回测预检未通过（阻断）",
                detail_zh="预检执行时抛出异常，回测被 fail-closed 阻断：" + str(exc),
                correlation_id=correlation_id,
                impact="本次回测请求被阻断，不创建 run；DB 自增 ID 不消耗",
                fix_link="/docs/precheck-requirements#precheck-runtime",
                retryable=False,
                blocking_reasons=blockers,
            ),
        ) from exc
    if precheck_resp.blocking_reasons:
        blockers_json = [b.model_dump(mode="json") for b in precheck_resp.blocking_reasons]
        first = blockers_json[0] if blockers_json else {}
        detail_txt = str(first.get("title_zh") or "预检发现阻断项")
        evidence = first.get("evidence") if isinstance(first, dict) else None
        if isinstance(evidence, dict) and "gap_days" in evidence:
            detail_txt = f"最低交易日不足，缺口 {evidence['gap_days']} 天"
        raise HTTPException(
            status_code=400,
            detail=_build_structured_error(
                error_code="PRECHECK_BLOCKED",
                title_zh="回测预检未通过（阻断）",
                detail_zh=detail_txt,
                correlation_id=correlation_id,
                impact="本次回测请求被阻断，不创建 run；DB 自增 ID 不消耗",
                fix_link="/docs/precheck-requirements#insufficient-trade-days",
                retryable=False,
                blocking_reasons=blockers_json,
            ),
        )
    # ------------------------------------------------------------
    ...
    try:
        ...
        # Task 12.1: 回写数据截止信息到 result dict
        if data_cutoff is not None:
            result["data_cutoff"] = data_cutoff.model_dump(mode="json")
        result["cutoff_mismatch"] = bool(cutoff_mismatch)
        result["cutoff_warning_zh"] = cutoff_warning_zh
        return PortfolioBacktestResult(**result)
    except ValueError as exc:
        msg = str(exc)
        mapping = [
            ("not found", 404, "BACKTEST_PORTFOLIO_NOT_FOUND", "组合不存在", "not-found"),
            ("auto_trade_enabled is 0", 409, "BACKTEST_AUTO_TRADE_DISABLED", "组合自动交易未开启", "auto-trade"),
            ("is not simulated", 409, "BACKTEST_PORTFOLIO_NOT_SIMULATED", "组合不是模拟账户，无法执行回测", "not-simulated"),
            ("total_capital is", 400, "BACKTEST_CAPITAL_INVALID", "组合初始资金配置非法", "capital"),
            ("Cannot run whole-portfolio backtest", 400, "BACKTEST_CANNOT_START", "组合回测前置条件不满足", "prerequisites"),
            ("manual/confirm 成员", 409, "BACKTEST_MEMBER_MODE_CONFLICT", "组合含 manual/confirm 成员，需切换 auto 模式", "member-mode"),
        ]
        status_code = 400
        error_code = "BACKTEST_INVALID_ARGUMENT"
        title_zh = "组合回测请求参数非法"
        fix_suffix = "precheck-requirements"
        for kw, sc, ec, tz, fs in mapping:
            if kw in msg:
                status_code, error_code, title_zh, fix_suffix = sc, ec, tz, f"precheck-requirements#{fs}"
                break
        raise HTTPException(
            ...
            detail=_build_structured_error(
                ...
                correlation_id=correlation_id,
                impact="本次回测请求被阻断，不创建 run；DB 自增 ID 不消耗",
                fix_link=f"/docs/{fix_suffix}",
                retryable=False,
            ),
        ) from exc
    except Exception as exc:
        retryable = _is_db_retryable(exc)
        logging.getLogger(__name__).exception("组合整体回测执行失败 correlation_id=%s", correlation_id)
        raise HTTPException(
            status_code=500,
            detail=_build_structured_error(
                error_code="UNKNOWN_BACKTEST_ERROR",
                title_zh="组合回测执行失败（未知错误）",
                detail_zh=str(exc) if retryable else "组合整体回测执行失败，请稍后重试",
                correlation_id=correlation_id,
                impact="本次回测未完成；若已创建 run 将标记为 failed 状态",
                fix_link="/docs/precheck-requirements#unknown-error",
                retryable=retryable,
            ),
        ) from exc
...
# ---------------- BFG 过滤治理：回测预检 ----------------
from app.schemas.bfg_precheck import BacktestPrecheckRequest, BacktestPrecheckResponse
from app.services.bfg_precheck_service import run_backtest_precheck


@router.post(
    "/backtest/precheck",
    response_model=BacktestPrecheckResponse,
    summary="回测预检（数据覆盖/过滤治理/最低交易日）",
)
def backtest_precheck(
    payload: BacktestPrecheckRequest,
    db: Session = Depends(get_db),
) -> BacktestPrecheckResponse:
    """提交前预检：返回最低交易日、可用交易日、目标覆盖、排除计数、阻断项/告警项。

    Task 12.2：失败一律返回 7 要素结构化错误，保证前端阻断 Modal 可以统一渲染。
    """
    correlation_id = _uuid.uuid4().hex
    try:
        return run_backtest_precheck(db, payload)
    except ValueError as exc:
        msg = str(exc)
        raise HTTPException(
            status_code=400,
            detail=_build_structured_error(
                error_code="BACKTEST_PRECHECK_INVALID_ARGUMENT",
                title_zh="回测预检参数非法",
                detail_zh=msg,
                correlation_id=correlation_id,
                impact="预检未能完成，无法给出阻断/告警清单",
                fix_link="/docs/precheck-requirements#invalid-argument",
                retryable=False,
            ),
        ) from exc
    except HTTPException as exc:
        # 业务层已经通过 HTTPException.detail 抛出结构化信息或旧字符串，保持细节
        if isinstance(exc.detail, dict) and "error_code" in exc.detail:
            structured = dict(exc.detail)
            structured.setdefault("correlation_id", correlation_id)
            structured.setdefault("title_zh", "回测预检失败")
            structured.setdefault("detail_zh", structured.get("message") or structured.get("reason") or str(exc.detail))
            structured.setdefault("impact", "预检未能完成，无法给出阻断/告警清单")
            structured.setdefault("fix_link", "/docs/precheck-requirements")
            structured.setdefault("retryable", False)
            raise HTTPException(status_code=exc.status_code, detail=structured) from exc
        raise HTTPException(
            status_code=exc.status_code,
            detail=_build_structured_error(
                error_code="BACKTEST_PRECHECK_FAILED",
                title_zh="回测预检失败",
                detail_zh=str(exc.detail) if exc.detail else "预检调用失败",
                correlation_id=correlation_id,
                impact="预检未能完成，无法给出阻断/告警清单",
                fix_link="/docs/precheck-requirements",
                retryable=False,
            ),
        ) from exc
    except Exception as exc:
        ...
        raise HTTPException(
            status_code=500,
            detail=_build_structured_error(
                error_code="UNKNOWN_BACKTEST_PRECHECK_ERROR",
                title_zh="回测预检执行失败（未知错误）",
                detail_zh=str(exc),
                correlation_id=correlation_id,
                impact="预检未能完成，无法给出阻断/告警清单",
                fix_link="/docs/precheck-requirements#unknown-error",
