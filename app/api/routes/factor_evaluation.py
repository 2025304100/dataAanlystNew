...
@router.get("/factor-evaluation/tasks/{task_id}/heartbeat")
def get_eval_task_heartbeat(task_id: str):
    """轻量只读心跳：不会触发巡检或过期任务写入。"""
    task = get_evaluation_task_heartbeat(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="evaluation_task_not_found")
    return task
