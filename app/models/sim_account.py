...
from sqlalchemy import DateTime, Double, Float, ForeignKey, Index, Integer, String, Text
...
class CashLedger(Base):
    ...
    # TD4（0057）：账务金额列用 Double（SQLAlchemy Float 无 precision 在 MySQL=单精度，
    # 1.2e9+0.01 级金额会被量化丢失分位以下精度）
    amount: Mapped[float] = mapped_column(Double)
    balance_after: Mapped[float] = mapped_column(Double)
...
class SimOrder(Base):
    ...
    filled_amount: Mapped[float] = mapped_column(Double, default=0)  # TD4（0057）账务金额列 Double
