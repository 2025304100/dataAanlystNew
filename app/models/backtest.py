...
from sqlalchemy import Date, DateTime, Double, Float, ForeignKey, Index, Integer, String, Text
...
class BacktestRun(Base):
    ...
    initial_capital: Mapped[float] = mapped_column(Double)  # TD4（0057）账务金额列 Double
    ...
    # -- BFG 过滤治理配置快照与保真标记 --
    filter_config_json: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="BacktestFilterConfig JSON 快照（运行时不可变）；NULL 为历史回测",
    )
    config_hash: Mapped[str | None] = mapped_column(
        String(64), nullable=True,
        comment="过滤配置 SHA256 hex 指纹，用于复现/对账",
    )
    production_fidelity: Mapped[int] = mapped_column(
        Integer, default=1, server_default="1",
        comment="BOOLEAN 0/1：1=生产保真(核心规则全开启)，0=非保真实验",
    )
    non_fidelity_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="非保真原因字符串，如 'filter_st disabled; filter_new_listing disabled'",
    )
...
class BacktestTrade(Base):
    ...
    # -- BFG 过滤治理 config_hash 联结 --
    config_hash: Mapped[str | None] = mapped_column(
        String(64), nullable=True,
        comment="对应 run 的 BacktestFilterConfig hash，独立可追溯",
    )
...
class BacktestExecutionFill(Base):
    ...
    config_hash: Mapped[str | None] = mapped_column(
        String(64), nullable=True,
        comment="对应 run 的 BacktestFilterConfig hash，保证逐笔可溯源",
    )
