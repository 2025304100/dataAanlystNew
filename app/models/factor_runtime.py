...
class FactorAuditLog(Base):
    '''AC-18 全写操作审计：before_json/after_json/actor/timestamp 非空，
    action ∈ {freeze, train, activate, fallback, clone_set, cancel, create, retire}。'''

    __tablename__ = 'factor_audit_logs'

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    action: Mapped[str] = mapped_column(String(64), index=True)
    # 稳定关联键；未使用该实体时允许空字符串，但 before/after/actor 必须非空
    factor_set_id: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    # 挖掘域：候选池 ID 落**独立列**（迁移 0055）。
    # 此前只在 attributes_json 里 → 按池查审计要走 JSON 过滤、无索引可用。
