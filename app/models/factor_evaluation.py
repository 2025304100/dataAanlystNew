...
class EvaluationRun(Base):
    ...
    filter_config_json: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="BacktestFilterConfig JSON 快照；NULL 为历史评价运行",
    )
    config_hash: Mapped[str | None] = mapped_column(
        String(64), nullable=True,
        comment="过滤配置 SHA256 hex 指纹，用于复现",
    )
    production_fidelity: Mapped[int] = mapped_column(
        Integer, default=1, server_default="1",
        comment="BOOLEAN 0/1：1=生产保真，0=非保真实验",
    )
    non_fidelity_reason: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="非保真原因字符串，如 'filter_st disabled'",
    )
    # 三计数：面板对齐前后有效样本数
    raw_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True,
        comment="面板原始行数（未过滤前）",
    )
    filtered_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True,
        comment="PIT 过滤后面板行数（分母）",
    )
    effective_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True,
        comment="最终计入 IC/分组/换手的有效行数（filtered - 标签缺失）",
    )
