...
class StrategyExecutionSnapshot(Base):
    ...
    # ── TD6 补列（对齐 DB 实态 SHOW CREATE TABLE，2026-09-18）────────────
    # 这三列在 DB 里是迁移体系之外的加宽（alembic 无任何 add_column 记录），
    # 历史上靠 tests/test_wp02 对新草案模型的 extend_existing 偶然合并才在
    # 测试进程内可见——生产进程没有它们，save_and_apply 必 TypeError。
    # 以 DB 实态为唯一权威声明：
    #   usage_binding_id  int NULL、**无 FK**（声明 FK 会制造 missing_fk 假漂移）
    #   snapshot_json     text NOT NULL
    #   content_hash      varchar(64) NOT NULL（与 snapshot_hash 两列共存，
    #                     同 idempotency 的 result_json/response_json 模式）
    usage_binding_id: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, index=True,
        comment="save_and_apply 绑定的 PortfolioFactorUsage.id（DB 实态 int 可空、无外键）",
    )
    snapshot_json: Mapped[str] = mapped_column(
        Text,
        default="{}",
        comment="pfu_snap_v1：save_and_apply 时冻结的 canonical JSON（portfolio/因子绑定/权重快照）",
    )
    content_hash: Mapped[str] = mapped_column(
        String(64), index=True, default="",
        comment="T4a canonical hash（=service content_hash(payload)）；与 snapshot_hash 两列共存",
    )
