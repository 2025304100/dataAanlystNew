...
from sqlalchemy import Date, DateTime, Double, Float, ForeignKey, Integer, UniqueConstraint
...
class PortfolioEquitySnapshot(Base):
    ...
    # 资产快照（TD4/0057：cash_balance/market_value 为账务金额列，Double 防单精度量化）
    cash_balance: Mapped[float] = mapped_column(Double, default=0)
    market_value: Mapped[float] = mapped_column(Double, default=0)
