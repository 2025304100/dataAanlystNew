# -*- coding: utf-8 -*-
"""T32 开工登记：writes 补登记（i18n）+ selfcheck + PROGRESS 立 in_progress。"""
import io
import json
import os
import subprocess
...
t32 = next(t for t in data["tasks"] if t["id"] == "T32")
before = list(t32["writes"])
for w in ADD:
    if w not in t32["writes"]:
        t32["writes"].append(w)
tpath.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print("writes 补登记:", before, "->", t32["writes"])
...
missing = [d for d in t32["deps"] if d not in done]
print("\nT32:", t32["title"], "| phase:", t32["phase"], "| gate:", t32["gate"])
print("not_do:", t32["not_do"])
print("pitfalls:", t32["pitfalls"])
print("未满足依赖:", missing or "NONE")
assert not missing
assert "T32" not in prog["tasks"]
...
print("\nPROGRESS.json T32 -> in_progress @", NOW)
