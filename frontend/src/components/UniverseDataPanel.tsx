...
export default function UniverseDataPanel() {
  ...
  const [industryTask, setIndustryTask] = useState<UniverseTask | null>(null);
  const [industryStarting, setIndustryStarting] = useState(false);
  const [industryCancelling, setIndustryCancelling] = useState(false);
  ...
  const refreshIndustryTask = useCallback(async () => {
    try {
      const data = await api.getUniverseIndustryBackfillStatus();
      setIndustryTask(data as UniverseTask | null);
    } catch {
      // ignore
    }
  }, []);
  ...
  useEffect(() => {
    ...
    refreshRepairTask();
    refreshIndustryTask();
  }, [refreshStats, refreshTask, refreshSmartTask, refreshIncrTask, refreshBfTask, refreshRepairTask, refreshIndustryTask]);
  ...
  useEffect(() => {
    if (!industryTask || (industryTask.status !== "running" && industryTask.status !== "queued")) {
      return;
    }
    const timer = setInterval(() => {
      refreshIndustryTask();
    ...
    return () => clearInterval(timer);
  }, [industryTask?.status, industryTask?.id, refreshIndustryTask, refreshStats]);
  ...
  const handleIndustryStart = async () => {
    setIndustryStarting(true);
    try {
      await api.startUniverseIndustryBackfill();
      await refreshIndustryTask();
      showToast("success", t("universeIndustryBackfillStarted"));
    } catch (e: any) {
      showToast("error", e.message || t("universeIndustryBackfillStartFailed"));
    } finally {
      setIndustryStarting(false);
    }
  };

  const handleIndustryCancel = async () => {
    setIndustryCancelling(true);
    try {
      await api.cancelUniverseIndustryBackfill();
      await refreshIndustryTask();
      showToast("success", t("universeIndustryBackfillCancelled"));
    } catch (e: any) {
      showToast("error", e.message || t("universeIndustryBackfillCancelFailed"));
    } finally {
      setIndustryCancelling(false);
    }
  };
  ...
  const industryIsRunning = industryTask?.status === "running" || industryTask?.status === "queued";
  // 任意同步任务运行中时，禁用其他启动按钮
  const anyRunning = smartIsRunning || isRunning || incrIsRunning || bfIsRunning || repairIsRunning || industryIsRunning;
      ...
      <section className="band">
        <div className="panel">
          ...
          <Card
            title={
              <Space>
                <span>{t("universeIndustryBackfillTitle")}</span>
                <Tag color="green">{t("universeIndustryBackfillBadge")}</Tag>
                {industryTask ? statusTag(industryTask.status) : null}
              </Space>
            }
            size="small"
            style={{ marginBottom: 16 }}
          >
            <Alert
              ...
              showIcon
              style={{ marginBottom: 12 }}
              message={t("universeIndustryBackfillDescription")}
            />
            {industryTask ? (
              <>
                <Progress
                  percent={Math.round(industryTask.percent || 0)}
                  status={industryTask.status === "failed" ? "exception" : industryTask.status === "done" ? "success" : industryTask.status === "cancelled" ? "exception" : "active"}
                />
                <p style={{ color: "var(--text-muted, #888)", fontSize: 12, margin: "8px 0 12px" }}>
                  {industryTask.message || t("universeIndustryBackfillPreparing")}
                  {industryTask.total > 0 ? `；${t("universeProcessed")} ${industryTask.processed}/${industryTask.total}，${t("universeIndustryBackfillUpdated")} ${industryTask.ok_count}` : ""}
                </p>
              </>
            ) : (
              <p style={{ color: "var(--text-muted, #888)", fontSize: 12 }}>{t("universeIndustryBackfillNoTask")}</p>
            )}
            <Space>
              {!industryIsRunning && (
                <Button
                  type="primary"
                  icon={<CloudSyncOutlined />}
                  onClick={handleIndustryStart}
                  loading={industryStarting}
                  disabled={anyRunning}
                >
                  {t("universeIndustryBackfillStart")}
                </Button>
              )}
              {industryIsRunning && (
                <Button danger icon={<StopOutlined />} onClick={handleIndustryCancel} loading={industryCancelling}>
                  {t("universeIndustryBackfillCancel")}
                </Button>
              )}
            </Space>
          </Card>
