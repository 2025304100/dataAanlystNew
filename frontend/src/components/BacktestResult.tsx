...
export default function BacktestResult({ result, portfolioId, onAppliedToPortfolio }: BacktestResultProps) {
  ...
  // Task 21.1: 非保真实验横幅
  const fidelityFlag = (result as unknown as Record<string, unknown>).production_fidelity;
  const fidelityBool =
    fidelityFlag === true ||
    fidelityFlag === 1 ||
    fidelityFlag === "1" ||
    fidelityFlag === "true";
  const nonFidelityReason = (result as unknown as Record<string, unknown>)
    .non_fidelity_reason as string | null | undefined;
