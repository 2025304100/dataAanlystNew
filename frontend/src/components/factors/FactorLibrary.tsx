...
export default function FactorLibrary({ onViewDetail, onEditFactor, onNewFactor }: FactorLibraryProps) {
        ...
        return (
          <Tooltip title={setCount + modelCount > 0 ? `已在 ${setCount} 个因子集 / ${modelCount} 个模型中使用，点击查看详情` : "查看该因子参与的因子集与模型"}>
            <Button
              ...
              data-testid={`btn-usage-${record.code}`}
            >
        ...
        <Spin spinning={usageLoading}>
          {!u && !usageLoading ? (
            ...
            <>
              ...
              {/* Task 6 (5): 使用情况 Usage Tab — 两个子 Tab：所属集合 / 关联模型 */}
              <div
                className="usage-sub-tabs"
                data-testid="usage-sub-tabs-wrapper"
                style={{ marginTop: 4 }}
              >
                <Tabs
                  size="small"
                  defaultActiveKey="sets"
                  data-testid="usage-sub-tabs"
                  items={[
                    {
                      key: "sets",
                      label: (
                        <Space size={4}>
                          <span>{isZh ? "所属集合" : "In Factor Sets"}</span>
                          <Tag color="cyan">{u.in_factor_sets?.length ?? 0}</Tag>
                        </Space>
                      ),
                      children: (
                        <div data-testid="usage-tab-sets" className="usage-tab-sets">
                          {(!u.in_factor_sets || u.in_factor_sets.length === 0) ? (
                            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={isZh ? "该因子暂未加入任何因子集合" : "Not in any factor set"} />
                          ) : (
                            <List
                              grid={{ gutter: 12, xs: 1, sm: 1, md: 2, lg: 2, xl: 2, xxl: 2 }}
                              dataSource={u.in_factor_sets}
                              renderItem={(s: ScoringFactorUsageSet, idx) => {
                                const statusColorMap: Record<string, string> = {
                                  active: "green", frozen: "gold", draft: "blue", retired: "default", deprecated: "default",
                                };
                                return (
                                  <List.Item key={`${s.factor_set_id}@${s.version}-${idx}`} data-testid={`usage-set-card-${s.factor_set_id}`}>
                                    <Card
                                      size="small"
                                      hoverable
                                      className="usage-set-card"
                                      onClick={() => navigate(`/factors?set_id=${encodeURIComponent(String(s.factor_set_id))}`)}
                                      style={{ cursor: "pointer", width: "100%" }}
                                      title={
                                        <Space size={6} wrap>
                                          <Text strong>{s.label || s.factor_set_id}</Text>
                                          <Tag color="blue" style={{ marginInlineEnd: 0 }}>v{s.version}</Tag>
                                        </Space>
                                      }
                                      extra={
                                        <Tag color={statusColorMap[s.status] || "default"} style={{ marginInlineEnd: 0 }}>
                                          {s.status}
                                        </Tag>
                                      }
                                    >
                                      <Space direction="vertical" size={4} style={{ minWidth: 0 }}>
                                        <div style={{ fontSize: 11, color: "#64748b" }}>
                                          ID: <Text code>{s.factor_set_id}</Text>
                                        </div>
                    ...
                    },
                    {
                      ...
                      label: (
                        <Space size={4}>
                          <span>{isZh ? "关联模型" : "In Models"}</span>
                          <Tag color="green">{u.in_models?.length ?? 0}</Tag>
                        </Space>
                      ),
                      children: (
                        <div data-testid="usage-tab-models" className="usage-tab-models">
                          {(!u.in_models || u.in_models.length === 0) ? (
                            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={isZh ? "该因子暂未出现在任何训练模型中" : "Not used in any trained model"} />
                          ) : (
                            <List
                              dataSource={u.in_models}
                              rowKey="model_id"
                              renderItem={(m: ScoringFactorUsageModel, idx) => {
                                const wPct = (m.normalized_weight ?? 0) * 100;
                                return (
                                  <List.Item key={`${m.model_id}-${idx}`} data-testid={`usage-model-card-${m.model_id}`} style={{ paddingInline: 0 }}>
                                    <Card
                                      size="small"
                                      hoverable
                                      className="usage-model-card"
                                      onClick={() => navigate(`/settings/factors?model_id=${encodeURIComponent(String(m.model_id))}`)}
                                      style={{ cursor: "pointer", width: "100%", borderRadius: 8 }}
                                    >
                                      <Row gutter={[12, 8]} align="middle">
                                        ...
                                        <Col flex="auto">
                                          <div style={{ fontSize: 12, color: "#475569", marginBottom: 4 }}>
                                            {isZh ? "模型快照权重（来自 snapshot）" : "Weight"}：
                                            <Text strong style={{ color: "#0f766e", marginInlineEnd: 8 }}>
                                              {wPct.toFixed(3)}%
                                            </Text>
                                          </div>
                                          <Progress
                                            percent={Number(Math.max(0, Math.min(100, wPct)).toFixed(1))}
                                            size="small"
                                        ...
                                        <Col flex="180px" style={{ textAlign: "right" }}>
                                          <Space direction="vertical" size={2} style={{ alignItems: "flex-end" }}>
                                            <Tag color={icColor(m.model_validation_ic ?? undefined)} style={{ marginInlineEnd: 0, fontSize: 11 }}>
                                              模型IC {m.model_validation_ic != null && Number.isFinite(m.model_validation_ic)
                                                ? (m.model_validation_ic * 100).toFixed(2) + "%"
                                                : "-"}
                                            </Tag>
                                            <Tag color={icColor(m.validation_ic ?? undefined)} style={{ marginInlineEnd: 0, fontSize: 11 }}>
                                              本因子IC {m.validation_ic != null && Number.isFinite(m.validation_ic)
                                                ? (m.validation_ic * 100).toFixed(2) + "%"
                                                : "-"}
                                            </Tag>
  ...
  const onApproveDraft = async (row: ScoringFactorDraft) => {
    const reviewer = ctx?.currentUser?.name ?? "superuser";
    try {
